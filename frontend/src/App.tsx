...
export default function App() {
  ...
  async function retrieve(event?: FormEvent) {
    ...
    try {
      const response = await request<Retrieval>('/api/v1/retrieve', controller.signal, { paper_id: selected.paper_id, query: question.trim(), top_k: topK, mode: retrievalMode, rerank });
      ...
      <main className="research-main">
        ...
        {selected ? <>
          ...
          <section className="question-section" aria-labelledby="question-heading"><div className="question-heading"><div><span className="section-number">01</span><h2 id="question-heading">向这篇论文提问</h2></div><span>检索范围：当前论文</span></div>
            ...
            </form><p className="query-hint"><Icon name="info" width="14" height="14" />支持词法、向量和混合检索对比。当前模型针对英文，建议输入英文问题；可选模型重排会增加等待时间；重排问题限 128 token，首次调用需要加载本地模型。</p>
          </section>
