...
function StorageView({ system, jobs, loading, error, jobsError, refresh }: {
  ...
  return <main className="storage-view">
    ...
    <p className="storage-note"><Icon name="info" />QASPER 与上传 PDF 共用工作台。文本 PDF 从论文库入口上传；当前上传为本机公共范围，尚不提供私有上传。</p>
  </main>;
}
...
export default function App() {
  ...
  const downloadKind = useRef(false);
  ...
  function selectPaper(paper: Paper) {
    ...
    if (paper.source === 'pdf') { setRetrievalMode('bm25'); setRerank(false); setAnswerMode(Boolean(system?.capabilities.generation)); }
  ...
  async function download(originalPdf = false) {
    if (!selected || downloading) return;
    downloadKind.current = originalPdf;
    const controller = new AbortController(); downloadController.current = controller;
    setDownloading(true); setDownloadError('');
    try { await downloadSource(selected.paper_id, controller.signal, originalPdf); }
  ...
  return <div className="app-shell">
    ...
    {tab === 'storage' ? <StorageView system={system} jobs={jobs} loading={systemLoading} error={systemError} jobsError={jobsError} refresh={() => setSystemRevision(value => value + 1)} /> : <div ref={workbenchRef} className={`workbench ${resizingPane ? 'resizing' : ''}`} style={{ '--library-width': `${libraryWidth}px` } as CSSProperties}>
      <aside className="paper-library" aria-label="论文库">
        <div className="library-heading"><div><p className="eyebrow">YOUR RESEARCH LIBRARY</p><h1>论文库 <span>{number(system?.corpus.papers)}</span></h1></div><span className="source-mark">论文</span></div>
        {system?.capabilities.pdf_upload && <PdfUpload key={credentialRevision} onOpen={paper => { selectPaper(paper); setSearch(''); setFilter(''); setDirection('all'); setOffset(0); setListRevision(v => v + 1); setSystemRevision(v => v + 1); }} />}
      ...
      <main className="research-main">
        ...
        {selected ? <>
          <section className="paper-overview" aria-labelledby="paper-heading"><div className="paper-overline"><span className="tag green">当前论文</span><span className="mono" title={selected.paper_id}>{selected.source === 'pdf' ? selected.paper_id.slice(0, 16) + '…' : selected.paper_id}</span>{selected.research_direction_label && <span className="direction-badge" title={`arXiv 主分类 ${selected.arxiv_primary_category}`}>{selected.research_direction_label} · {selected.arxiv_primary_category}</span>}{selected.source !== 'pdf' && <span>{paperDate(selected.arxiv_submitted_at)} 首次提交 arXiv</span>}{selected.ccf_level && <span className={`ccf-badge ccf-${selected.ccf_level.toLowerCase()}`} title="CCF 对发表载体的目录级别；不代表论文质量评分">{selected.ccf_venue} · CCF {selected.ccf_level}</span>}<span className="paper-version" title={selected.version}>{selected.source === 'pdf' ? '文本解析 v1' : selected.version}</span></div><h1 id="paper-heading" lang="en">{selected.title}</h1>
            ...
            <div className="paper-actions"><button className="text-button" onClick={() => setAbstractExpanded(value => !value)} aria-expanded={abstractExpanded}>{abstractExpanded ? '收起摘要' : '展开摘要'}<Icon name="chevron" className={abstractExpanded ? 'rotate-up' : 'rotate-down'} width="13" height="13" /></button>{selected.source === 'pdf' ? <button className="text-button" onClick={() => void download(true)} disabled={downloading}>下载上传的原 PDF</button> : selected.arxiv_pdf_url ? <a className="text-button" href={selected.arxiv_pdf_url} target="_blank" rel="noreferrer" title="从 arXiv 打开原论文 PDF，可在 PDF 阅读器中下载"><Icon name="external" width="14" height="14" />打开/下载原论文 PDF</a> : <button className="text-button" disabled title="当前记录没有可核验的原论文 PDF 链接"><Icon name="external" width="14" height="14" />原论文 PDF 暂无</button>}<button className="text-button" onClick={() => void download()} disabled={downloading || system?.object_store.status !== 'ready'} title={system?.object_store.status !== 'ready' ? '对象存储连接后可下载' : '下载已校验的 RAG 结构化正文 JSON'}><Icon name="download" width="15" height="15" />{downloading ? '正在下载…' : '下载 RAG 结构化正文'}</button></div>
            {selected.source === 'pdf' && <div className="pdf-notice"><p>上传 PDF · {selected.pdf_page_count} 页 · 当前支持词法检索，英文关键词更适合现有检索器。引用中的页码是 PDF 文件页序，不是印刷页码。</p>{selected.pdf_parse_warnings?.map((warning, index) => <p key={index}>{warning}</p>)}</div>}
          ...
          {!answerMode && <p className="run-record-hint">当前任务只检索原文，不生成答案；需要回答时，请选择“生成并核验回答”。</p>}
