...
面向 AI 论文阅读的可信 RAG 与受控反馈改进系统，使用 QASPER 正文建立可核对的证据链。P2A 已完成；当前已完成 **P2B-2：可选模型重排与六组消融**。工作台保留方向、时间/编号/CCF 排序、可调整分栏、PDF 入口和原文上下文，提供三种检索方式和默认关闭的“模型重排”开关。本地 BGE 向量必须完成索引构建才能查询；当前仍只返回原文证据，**尚未生成答案或完成答案核验**。
