...
面向 AI 论文阅读的可信 RAG 与受控反馈改进系统，使用 QASPER 正文建立可核对的证据链。按阶段实现，当前交付 **P2A：持久化论文检索工作台**。它能按 arXiv 主分类选择研究方向，再按编号、标题、首次提交时间或 CCF 发表载体等级排序论文；左右阅读区域可以拖动调整并记住比例；可以打开/下载官方 arXiv PDF、检索正文段落、查看相邻上下文、下载经过哈希校验的 RAG 结构化正文，并显示真实导入与存储状态。当前只返回 BM25 原文证据，**尚未生成答案或完成答案核验**。
