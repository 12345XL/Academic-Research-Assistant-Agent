...
页面支持论文列表/标题搜索、研究方向筛选、方向内服务端排序、官方 arXiv PDF 入口、单篇论文证据检索、top-k 选择、原文相邻段落查看、RAG 结构化正文下载，以及数据库、对象存储、导入任务状态。方向来自 arXiv 官方主分类；排序可按编号、标题、arXiv 首次提交时间或 CCF 发表载体等级选择，并在分页前执行。桌面端左右区域通过可访问的分隔线调整占比，支持鼠标/触控笔拖动、键盘微调、双击复位和本地持久化；手机保持上下布局。
...
| 方向与原论文 | 1,169 篇均有 arXiv 主分类和官方 PDF 链接；聚合为 10 个研究方向 |
| 离线全量回归 | 冻结 P1 的 745 道 dev 文本证据题逐题比较 top-5 段落 ID，排名差异 0 |
| 后端单元测试 | 81 通过；5 个需真实存储的用例按设计跳过 |
| 真实 PostgreSQL/S3 集成测试 | 合计 86 通过；覆盖方向筛选、PDF 元数据返回、元数据写入、导入幂等、缺失对象修复、数据库失败回滚、锁竞争、发布版本刷新和依赖故障 |
| 前端测试与构建 | 13 通过；TypeScript 检查和 Vite 生产构建成功 |
| 浏览器联调 | 实测先选方向再排序、官方 PDF 链接和左右分隔线；键盘从 230 调到 294 像素，刷新后仍为 294；论文检索与存储状态仍可用 |
| 重启恢复 | 停止并重新启动本机 PostgreSQL 与 SeaweedFS 后，已导入数据、下载和检索仍可用 |

排序元数据审计见 [paper_sort_metadata.json](../reports/paper_sort_metadata.json)。方向分布为自然语言处理 953、机器学习 47、信息检索 40、人工智能 34、社会计算 29、语音与音频 27、视觉与多媒体 24、其他 9、人机交互 4、机器人 2。PDF 按冻结快照中的 arXiv 官方链接打开，外部站点可用性和 PDF 字节不属于本地对象存储验收；具体边界见[数据契约](DATASET.md#21-浏览与排序元数据)。
