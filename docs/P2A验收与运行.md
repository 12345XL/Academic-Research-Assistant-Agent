...
页面支持论文列表/标题搜索、研究方向筛选、方向内服务端排序、官方 arXiv PDF 入口、单篇论文证据检索、top-k 选择、原文相邻段落查看、RAG 结构化正文下载，以及数据库、对象存储、导入任务状态。方向来自 arXiv 官方主分类；排序可按编号、标题、arXiv 首次提交时间或 CCF 发表载体等级选择，并在分页前执行。切换论文、方向或排序会清除前一篇的检索结果；较早的请求可取消。
...
| 前端测试与构建 | 12 通过；TypeScript 检查和 Vite 生产构建成功 |
| 浏览器联调 | 实测先选方向再按时间/CCF 排序，并核对官方 PDF 链接；论文检索、上下文弹窗和存储状态仍可用 |
